...
- **GIVEN** 系统已展示提交信息并进入交互菜单
- **WHEN** 用户选择"编辑"
- **THEN** 系统允许用户修改提交信息，修改后重新进行格式校验并回到交互菜单
...
#### Scenario: 鉴权类错误不重试
- **GIVEN** 配置的 API Key 无效
- **WHEN** LLM 返回鉴权/权限类错误
- **THEN** 系统立即停止重试并降级，不空耗剩余重试次数
